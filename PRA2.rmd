...
**Clase**
```{r}
ggplot(titanic_data, aes(x=Pclass, fill=Survived)) + geom_bar() 
```

Observamos una supervivencia claramente superior en pasajeros que viajaban en primera clase con respecto a aquellos que viajaban en tercera clase. Podemos concluir que la clase en la que los pasajeros viajaban fue determinante para su supervivencia.

**Género**
```{r}
ggplot(titanic_data, aes(x=Sex, fill=Survived)) + geom_bar()
```

Observamos que las mujeres tuvieron un porcentaje de supervivencia significativamente superior al de los hombres, por lo que podemos concluir que esta variable es determinante para la supervivencia.

**Rango de edad**
```{r}
titanic_data$AgeGroup <- cut(titanic_data$Age, breaks = c(0,15,25,55,75,100), labels = c("Children","Young","Adults","Retired","Elderly"))
ggplot(titanic_data, aes(x=AgeGroup, fill=Survived)) + geom_bar()
```
...
El rango de edad fue, por tanto, determinante para la supervivencia.

**Acompañantes**
```{r}
titanic_data$Companions <- titanic_data$SibSp + titanic_data$Parch
ggplot(titanic_data, aes(x=Companions, fill=Survived)) + geom_bar()
```
No se observa una diferencia significativa en el porcentaje de supervivencia en función del número de acompañantes, por lo que entendemos que esta variable no fue relevante.

**Puerto de embarque**
```{r}
ggplot(titanic_data, aes(x=Embarked, fill=Survived)) + geom_bar()
```

Observamos proporciones inferiores en la supervivencia de aquellos pasajeros embarcados en Southampton con respecto a los pasajeros embarcados en los otros dos puertos. Queda por estudiar si la diferencia es significativa.
...
La variable Age se aproxima ligeramente a una distribución normal, pero el resto de variables no. 
...
La probabilidad, p-value, es menor que el valor de significación, 0.05, por lo que se acepta la hipótesis nula del test y se puede afirmar que la variable Age no sigue una distribución normal
...
Para dar respuesta empleamos contrastes de hipótesis entre Pclass, Sex, AgeGroup y Survived: 
...
Observamos que embarcar en los puertos de Queenston o Southampton tiene una relación significativamente negativa con la supervivencia )p-value inferior a 0,05). 
